...
export async function runRotationSweep(env: Bindings, now = new Date()) {
  ...
  for (const group of allGroups) {
    ...
    // Each group is independent — one group's DB/network hiccup shouldn't
    // stop the rest of the sweep from rotating on time, so a failure here
    // is recorded and swept past rather than left to abort the `for` loop.
    try {
      ...
      failures.push({ groupId: group.id, error });
      console.error(`rotation sweep: group ${group.id} failed`, error);
    }
  }
...
export async function runPurgeSweep(env: Bindings, now = new Date()) {
  ...
  const failures: Array<{ selectionId: string; error: unknown }> = [];

  for (const row of due) {
    // Same reasoning as the rotation sweep: one row's blob/DB failure
    // shouldn't stop every other expired selection from being purged —
    // it's swept past and retried on the next hourly run instead.
    try {
      ...
      failures.push({ selectionId: row.selectionId, error });
      console.error(`purge sweep: selection ${row.selectionId} failed`, error);
    }
  }
...
export async function runScheduled(env: Bindings, now = new Date()) {
  // The two sweeps are independent (rotation picks today's photo; purge
  // deletes yesterday's) — one failing entirely (e.g. the DB being briefly
  // unreachable when its opening query runs) shouldn't stop the other from
  // getting its chance this hour. Both errors are still surfaced together
  // afterward so the Cron Trigger invocation is reported as failed, same as
  // before this per-sweep isolation was added.
  const errors: unknown[] = [];

  try {
    await runRotationSweep(env, now);
  } catch (error) {
    console.error("rotation sweep failed entirely", error);
    errors.push(error);
  }
  ...
  try {
    ...
    console.error("purge sweep failed entirely", error);
    errors.push(error);
  }

  if (errors.length > 0) {
    throw new AggregateError(errors, "runScheduled: one or more sweeps failed entirely");
  }
}
